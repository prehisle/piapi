...
当前版本的 `piapi` 路由策略是 **用户级别** 的，即一个用户的所有请求都会被路由到其绑定的唯一一个服务商。这导致我们无法为一个用户同时提供来自不同服务商的服务（例如，`codex` 来自服务商A，`claude_code` 来自服务商B）。
...
*   新增一个名为 `services` 的映射（map）。
    *   该映射的 **键 (key)** 是服务类型字符串（如 `"codex"`, `"claude_code"`）。
...
*   **路由解析逻辑变更**:
    1.  当网关收到一个请求时（例如 `.../piapi/codex/...`），首先像现在一样认证用户。
    2.  认证成功后，从请求URL中解析出服务类型（`"codex"`）。
...
*   根据上述新配置，用户 `zdy` 对 `.../piapi/codex` 的请求被正确路由到 `xychatai`。
*   用户 `zdy` 对 `.../piapi/claude_code` 的请求被正确路由到 `88code`。
